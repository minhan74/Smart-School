...
void Execute_Command() {
  ...
  if (data=="off"){
    if(digitalRead(MICRO_PIN)&&digitalRead(REMOTE_PIN)) {publishNow(facility, "good");}//end if
    else {publishNow(facility, "bad");}
    digitalWrite(MAIN_RELAY,LOW);
    roomON=false;
    } //off
...
void publishNow(Adafruit_MQTT_Publish topicPub,const char* MQTTmessage) {
  #if not defined (DEBUG_BASIC) || not defined (DEBUG_ADVANCE)
  topicPub.publish(MQTTmessage);
  #endif
  #if defined (DEBUG_BASIC) || defined (DEBUG_ADVANCE)
  char pub=0;
  if (!topicPub.publish(MQTTmessage)) {Serial.println(F("Failed!"));}//
  else                                {Serial.println(F("Published!"));}
  /*
  char pub=0;
  while (!topicPub.publish(MQTTmessage)) {Serial.println(F("Failed!")); if (pub++>3) break;}//
